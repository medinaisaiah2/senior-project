...
from support_resistance import get_SR
tzone = timezone('EST')
...
def trader(x):
    ...
    maximums, minimums = get_SR(stock)
    max = maximums[0]
    max = max * 0.95
    if (((quote['price'] < max) and (quote['price'] > minimums[0]) and (max-minimums[0] > std30)) and not x.havepos):
    ...
    elif (quote['price'] > max and x.havepos):
...
def performtrade(x,day):
    ...
    max = maximums[0]
    max = max * 0.95
    if(((stock.iloc[day]['Open'] < max) and (stock.iloc[day]['Open'] > minimums[0] and ((max - minimums[0] > std30)))) and not x.havepos):
        print("YES current day is below std\nBUY")
        #print(intc.iloc[day])
        #print(ravg30 - std30)
        ...
        if(current < x.paperdough):
            ...
            databasebuffer.append(
                {"stock":stockt,
                 ...
                 "amount":x.mystocks,
                 "total_price":-1*x.mystocks*current})
    elif (stock.iloc[day]['Open'] > max and x.havepos):
        print("YES current day is above std\nSELL")
        current = stock.iloc[day]['Open']
        databasebuffer.append(
            {"stock":stockt,
